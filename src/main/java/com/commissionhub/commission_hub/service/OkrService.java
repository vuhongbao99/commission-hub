...
public class OkrService {
    ...
    @Transactional
    public Okr createOrUpdate(Integer year, String targetType, String targetId,
                              Long revenueTarget, Integer taskTarget, Long commissionTarget) {
        User director = getCurrentUser();
        Okr okr = okrRepository.findByYearAndTargetTypeAndTargetId(year, targetType, targetId)
                .orElse(Okr.builder().year(year).targetType(targetType)
                        .targetId(targetId).createdBy(director).build());
    ...
    public Map<String, Object> getProgress(Integer year, String userId) {
        Okr okr = okrRepository.findByYearAndTargetTypeAndTargetId(year, "USER", userId)
                .orElseThrow(() -> new NotFoundException("Không tìm thấy OKR"));
        Long actualRevenue = commissionRepository.sumAmountByUserAndYear(userId, year);
